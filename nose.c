    ...
    if(argc==4){
        ...
        if(p[1]=='p' && (strcmp(mode_query,"Major")==0 || strcmp(mode_query,"Minor")==0)){
            ...
            while(aux!=NULL){
                ...
                if(aux2!=NULL){
                    if(strcmp(mode_query, aux2->Mode)==0){
                        while(data != NULL){
                            if (strcmp(data->Gender, aux->Gender)){
                                data_aux->counter ++;
                                check = true;
                            }
                        }
                        if (check == false){
                            if (data == NULL){
                                data = (Modo*)malloc(sizeof(Modo));
                                data->Gender = aux->Gender;
                                data->counter = 1;
                                data->next = NULL;
                            }
                            else{
                                data_aux = data;
                                while (data_aux->next != NULL) {
                                    data_aux = (Modo*)data_aux->next;
                                }
                            }
                            data_aux = data;
                        }
            ...
            while (data != NULL) {
                printf("%s %d\n",data_aux->Gender,data_aux->counter);
            }
        }
        else{
            printf("Error: the query is invalid\n");
